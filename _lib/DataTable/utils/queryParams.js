...
selectedFilter, filterValue) {
  ...
  if (ccSelectedFilter === "startsWith") {
    ...
    return qb ? qb.between(new Date(_filterValueToUse2[0]), new Date(_filterValueToUse2[1])) : function (fieldVal) {
      return (0, _moment2.default)(_filterValueToUse2[0]).valueOf() <= (0, _moment2.default)(fieldVal).valueOf() && (0, _moment2.default)(fieldVal).valueOf() <= (0, _moment2.default)(_filterValueToUse2[1]).valueOf();
    };
...
function getCurrentParamsFromUrl(location) {
  ...
  return parseFilters(_queryString2.default.parse(search));
}
function setCurrentParamsOnUrl(newParams, replace) {
  var stringifiedFilters = stringifyFilters(newParams);
  replace({
    search: "?" + _queryString2.default.stringify(stringifiedFilters)
  });
}
...
function stringifyFilters(newParams) {
  ...
  if (newParams.filters && newParams.filters.length) {
    filters = newParams.filters.reduce(function (acc, _ref, index) {
      ...
      acc += (index > 0 ? "___" : "") + (filterOn + "__" + (0, _lodash.camelCase)(selectedFilter) + "__" + safeStringify(Array.isArray(filterValue) ? filterValue.join(".") : filterValue));
      return acc;
    }, "");
  }
  var order = void 0;
