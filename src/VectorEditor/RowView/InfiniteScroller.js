...
class InfiniteScoller extends React.Component {
  ...
  isRowActuallyVisible = row => {
    ...
    const potentiallyVisibleRow =
      potentiallyVisibleRows.children[rowIndexInVisibleRowsContainer];
    ...
    if (
      (icTop < pvBottom && icBottom > pvTop) ||
      (icBottom > pvTop && icTop < pvBottom)
    ) {
      return true;
    }
    return false;
  ...
  onEditorScroll = event => {
    ...
    const visibleRowsContainer = ReactDOM.findDOMNode(
      this.refs.visibleRowsContainer
    );
    ...
    if (distanceFromTopOfVisibleRows < 0) {
      if (this.rowStart > 0) {
        rowsToAdd = Math.ceil(
          -1 * distanceFromTopOfVisibleRows / this.props.averageElementHeight
        );
      ...
      if (rowsToGiveOnBottom > 0) {
        rowsToAdd = Math.ceil(
          -1 * distanceFromBottomOfVisibleRows / this.props.averageElementHeight
        );
        newRowStart = this.rowStart + rowsToAdd;

        if (
          newRowStart + this.state.visibleRows.length >=
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      ...
      this.prepareVisibleRows(
        rowStart,
        newNumberOfRowsToDisplay,
        nextProps.totalNumberOfRows
      );
    }
  }

  componentWillUpdate() {
    let visibleRowsContainer = ReactDOM.findDOMNode(
      this.refs.visibleRowsContainer
    );
  ...
  componentDidUpdate() {
    ...
    function adjustScrollHeightToRowJump() {
      ...
      if (rowJumpedToIndex === -1) {
        console.warn(
          "rowJumpedTo did not work out for row #:",
          this.rowJumpedTo
        );
        rowJumpedToIndex = 0;
      }
    ...
    if (
      ...
      if (
        this.rowStart + this.state.visibleRows.length <
        this.props.totalNumberOfRows
      ) {
  ...
  componentWillMount() {
    ...
    if (
      ...
      this.props.rowToJumpTo.row < this.props.totalNumberOfRows
    ) {
  ...
  prepareVisibleRows = (
    rowStart,
    newNumberOfRowsToDisplay,
    newTotalNumberOfRows
  ) => {
    // note, rowEnd is optional
    this.numberOfRowsToDisplay = newNumberOfRowsToDisplay;
    const totalNumberOfRows = areNonNegativeIntegers([newTotalNumberOfRows])
      ? newTotalNumberOfRows
      : this.props.totalNumberOfRows;
  ...
  render() {
    ...
    this.bottomSpacerHeight =
      (this.props.totalNumberOfRows - 1 - this.rowEnd) * rowHeight;
