...
function Labels({
  ...
  var labelPoints = Object.keys(labels)
    .map(function(key) {
...
function LabelGroup({ label, HoverHelper, key, ...rest }) {
  ...
  return (
    ...
    (
      <HoverHelper mouseAware={true} key={key} id={labelIds}>
        <DrawLabelGroup
          key={key}
          {...{
            label,
...
function DrawLabelGroup(props) {
  ...
  var maxLabelLength = labelAndSublabels.reduce(function(
    currentLength,
    { text }
  ) {
