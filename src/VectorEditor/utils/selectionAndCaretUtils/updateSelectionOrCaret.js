...
export default function updateSelectionOrCaret({
  ...
  if (shiftHeld) {
    if (caretPosition > 0) {
      ...
      if (newCaret > -1) {
        //new caret passed
        var distanceFromStart = getMinRangeLength(
          selectionLayer.start,
          newCaret,
          sequenceLength
        );
        var distanceFromEnd = getMinRangeLength(
          selectionLayer.end,
          newCaret,
          sequenceLength
        );
        ...
        //new range passed
        var selectionFullyContained = !trimRangeByAnotherRange(
          selectionLayer,
          newRange
        );
        ...
        var newRangeFullyContained = !trimRangeByAnotherRange(
          newRange,
          selectionLayer
        );

        var { newRange: range1 } = expandOrContractRangeToPosition(
          selectionLayer,
          newRange.start,
          sequenceLength
        );
        var { newRange: range2 } = expandOrContractRangeToPosition(
          selectionLayer,
          newRange.end + 1,
          sequenceLength
