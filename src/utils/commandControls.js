...
export const withCommand = mappings => WrappedComponent => ({ cmd, cmdOptions = {}, ...props }) => {
  const mappedProps = {};
  Object.keys(mappings).forEach(k => {
    mappedProps[k] = mappings[k] === "execute"
  ...
  let tooltip = cmd.tooltip || (typeof cmd.isDisabled === "string" && cmd.isDisabled);
...
export const CmdCheckbox = withCommand({
  onChange: "execute",
  label: (cmd, props) => (props.prefix && (<React.Fragment>{props.prefix}{cmd.name}</React.Fragment>)) || cmd.name,
  disabled: "isDisabled",
  checked: "isActive"
})(Checkbox);
