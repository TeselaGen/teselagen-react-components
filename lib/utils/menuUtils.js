...
exports.addMenuHandlers = exports.addMenuHotkeys = exports.EnhancedMenuItem = undefined;
...
var _hotkeyUtils = require("./hotkeyUtils");
...
// `navTo` prop
var EnhancedMenuItem = (0, _recompose.compose)((0, _recompose.lifecycle)({
  ...
  return navTo;
}, _reactRouterDom.withRouter))(function (_ref2) {
  ...
  return _react2.default.createElement(_core.MenuItem, _extends({}, props, { onClick: clickHandler }));
});

// Populate the given menu definition with any defined hotkeys, matched using
// the `cmd` property
exports.EnhancedMenuItem = EnhancedMenuItem;
var addMenuHotkeys = exports.addMenuHotkeys = function addMenuHotkeys(menuDef, hotkeys) {
  return walkMenu(menuDef, function (item) {
    var out = _extends({}, item);
    if (item.cmd && hotkeys[item.cmd]) {
      var props = (0, _hotkeyUtils.getHotkeyProps)(hotkeys[item.cmd], item.cmd);
...
// the `cmd` property
var addMenuHandlers = exports.addMenuHandlers = function addMenuHandlers(menuDef, handlers) {
  return walkMenu(menuDef, function (item) {
    var out = _extends({}, item);
    if (item.cmd && handlers[item.cmd]) {
      out.text = item.text || (0, _lodash.startCase)(item.cmd);
